% mochi_midairSlice.m 
% Slicing Mochi Ice Cream in Midair (no gravity)
...
% Viscoelastic constants (Ice Cream, cold)
md_p_cream = 1;             % particle mass density
mu_cream = 1.0;             % shear modulus
La_cream = 10;              % Lame's 1st parameter
mu_N_cream = 1.0*10^-2;     % viscous Newton constant
Wi_cream = 0.3;             % Weissenberg number

% Viscoelastic constants (Mochi, cold)
...
% ======================== KINEMATIC BLADES ============================== 
% Blade constants
hb = 0.25;  % blade node spacing & thickness
a = 4;      % ellipse long "radius" (scaled with hb)
...
% MPM Initial Values (k=0)
    ...
    % Particle Velocity (3xNP) (ice cream & mochi)              
    Uk = [v1x*ones(1,NP); 
          v1y*ones(1,NP);
          v1z*ones(1,NP)];    
    ...
    B1X = [blade1.x_co(:,1), blade1.x_co(:,1)];
    B1Y = [blade1.x_co(:,2), blade1.x_co(:,2)];
    B1Z = [blade1.x_co(:,3), blade1.x_co(:,3)];
    B2X = [blade2.x_co(:,1), blade2.x_co(:,1)];
    B2Y = [blade2.x_co(:,2), blade2.x_co(:,2)];
    B2Z = [blade2.x_co(:,3), blade2.x_co(:,3)];
    B3X = [blade3.x_co(:,1), blade3.x_co(:,1)];
    B3Y = [blade3.x_co(:,2), blade3.x_co(:,2)];
    B3Z = [blade3.x_co(:,3), blade3.x_co(:,3)];
...
%     dtk = min(dtk,tf-t);
    ...
    [Idx1] = CollisionDetect_Blade(Xg,blade1);
    [Idx2] = CollisionDetect_Blade(Xg,blade2);
    [Idx3] = CollisionDetect_Blade(Xg,blade3);
    [v_up1] = CollisionHandling(Idx1,Xg(Idx1,:),w(:,Idx1),blade1);
    [v_up2] = CollisionHandling(Idx2,Xg(Idx2,:),w(:,Idx2),blade2);
    [v_up3] = CollisionHandling(Idx3,Xg(Idx3,:),w(:,Idx3),blade3);
    ...
    [Idx1] = CollisionDetect_Blade(Phi_up,blade1); 
    [Idx2] = CollisionDetect_Blade(Phi_up,blade2); 
    [Idx3] = CollisionDetect_Blade(Phi_up,blade3);
    [v_up1] = CollisionHandling(Idx1,Phi_up(Idx1,:),Vk(:,Idx1),blade1);
    [v_up2] = CollisionHandling(Idx2,Phi_up(Idx2,:),Vk(:,Idx2),blade2);
    [v_up3] = CollisionHandling(Idx3,Phi_up(Idx3,:),Vk(:,Idx3),blade3);
    ...
    PHZ = [PHZ, Phi_up(:,3)];
    JPK = [JPK, det_3d(Fk)'];
    B1X = [B1X, blade1.x_co(:,1)];
    B1Y = [B1Y, blade1.x_co(:,2)];
    B1Z = [B1Z, blade1.x_co(:,3)];
    B2X = [B2X, blade2.x_co(:,1)];
    B2Y = [B2Y, blade2.x_co(:,2)];
    B2Z = [B2Z, blade2.x_co(:,3)];
    B3X = [B3X, blade3.x_co(:,1)];
    B3Y = [B3Y, blade3.x_co(:,2)];
    B3Z = [B3Z, blade3.x_co(:,3)];
...
% Write Blade 1 node positions to text file 
writematrix(B1X,'B1X.dat','Delimiter',';');  % (x-coords) (NObj x k+1)
writematrix(B1Y,'B1Y.dat','Delimiter',';');  % (y-coords) (NObj x k+1)
writematrix(B1Z,'B1Z.dat','Delimiter',';');  % (z-coords) (NObj x k+1)

% Write Blade 2 node positions to text file 
writematrix(B2X,'B2X.dat','Delimiter',';');  % (x-coords) (NObj x k+1)
writematrix(B2Y,'B2Y.dat','Delimiter',';');  % (y-coords) (NObj x k+1)
writematrix(B2Z,'B2Z.dat','Delimiter',';');  % (z-coords) (NObj x k+1)

% Write Blade 3 node positions to text file 
writematrix(B3X,'B3X.dat','Delimiter',';');  % (x-coords) (NObj x k+1)
writematrix(B3Y,'B3Y.dat','Delimiter',';');  % (y-coords) (NObj x k+1)
writematrix(B3Z,'B3Z.dat','Delimiter',';');  % (z-coords) (NObj x k+1)

% Write J=det(Fp) particle data to text file
writematrix(JPK,'Jpk.dat','Delimiter',';');  % (NP x k+1)

% -------------------------- ANIMATED SIM --------------------------------
% SAVE Workspace and IMPORT into mochi_midairSlice_ani.m  
