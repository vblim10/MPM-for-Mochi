/*==========================================================
 * G2P_PIC_2d.c - grid to particle interpolation
 *
...
void G2P(const double Ox, const double Oy,      // constants
         ...
         double *Xp, double *Yp, double *fg,    // input
         double bx, double by,
         double *fp                             // output       
         )
{
    ...
    for (int pidx=0; pidx<NP; pidx++) {
        ...
        for (int j=0; j<49; j++) {
            ...
            if (k>=0 && k<nx+1 && l>=0 && l<ny+1) {
                ...
                // PIC transfer
                fp_curr += fg[gidx]*( N_weight2(dx,bx) * N_weight2(dy,by) )/ wh; 
            }
        }
        fp[pidx] = fp_curr;
    }
                  ...
                  int nrhs, const mxArray *prhs[])
{
   ...
   /* check for proper number of arguments */
    if (nrhs!=11) {
        mexErrMsgIdAndTxt("G2P_PIC_2d:nrhs","11 inputs required.");
    }
    if (nlhs!=1) {
        mexErrMsgIdAndTxt("G2P_PIC_2d:nlhs","1 output required.");
    }

    /* make sure the 7th input argument is type double */
    if (!mxIsDouble(prhs[6]) ||  mxIsComplex(prhs[6])) {
        mexErrMsgIdAndTxt("G2P_PIC_2d:notDouble","Xp must be type double.");
    }

    /* check that number of cols in input arguments is 1 */
    if (mxGetN(prhs[6])!=1 || mxGetN(prhs[7])!=1 ) {
        mexErrMsgIdAndTxt("G2P_PIC_2d:notRowVector","Xp, Yp must be column vectors.");
    }
   
    /* check that number of rows in input arguments is 1 */
    if (mxGetM(prhs[8])!=1) {
        mexErrMsgIdAndTxt("G2P_PIC_2d:notRowVector","fg must be a row vector.");
    }
    ...
    if (mxGetNumberOfElements(prhs[0])!=1 || mxGetNumberOfElements(prhs[1])!=1 ||
        ...
        mexErrMsgIdAndTxt("G2P_PIC_2d:notScalar","Inputs Ox, Oy, hx, hy, nx, ny, bx, by must be scalars.");
    }
    ...
    /* create a pointer to the real data in Xp, Yp, fg */
