/*==========================================================
 * P2G_vec_2d.c - grid to particle interpolation
 *
 * Interpolates particle values fp1, fp2
 *               to grid values fg1, fg2
 *
                  ...
                  int nrhs, const mxArray *prhs[])
{
   ...
   /* check for proper number of arguments */
    if (nrhs!=12) {
        mexErrMsgIdAndTxt("P2G_vec_2d:nrhs","12 inputs required.");
    }
    if (nlhs!=2) {
        mexErrMsgIdAndTxt("P2G_vec_2d:nlhs","2 outputs required.");
    }

    /* make sure the 7th input argument is type double */
    if (!mxIsDouble(prhs[6]) ||  mxIsComplex(prhs[6])) {
        mexErrMsgIdAndTxt("P2G_vec_2d:notDouble","Xp must be type double.");
    }

    /* check that number of cols in input arguments is 1 */
    if (mxGetN(prhs[6])!=1 || mxGetN(prhs[7])!=1 ) {
        mexErrMsgIdAndTxt("P2G_vec_2d:notRowVector","Xp, Yp must be a column vectors.");
    }
   
    /* check that number of rows in input arguments is 1 */
    if (mxGetM(prhs[8])!=1 || mxGetM(prhs[9])!=1 ) {
        mexErrMsgIdAndTxt("P2G_vec_2d:notRowVector","fp1, fp2 must be a row vectors.");
    }
    ...
    if (mxGetNumberOfElements(prhs[0])!=1 || mxGetNumberOfElements(prhs[1])!=1 ||
        ...
        mexErrMsgIdAndTxt("P2G_vec_2d:notScalar","Inputs Ox, Oy, hx, hy, nx, ny, bx, by must be scalars.");
    }
    ...
    /* create a pointer to the real data in Xp, Yp, fp1, fp2 */
    ...
    fp1 = mxGetDoubles(prhs[8]);
    fp2 = mxGetDoubles(prhs[9]);
    #else
