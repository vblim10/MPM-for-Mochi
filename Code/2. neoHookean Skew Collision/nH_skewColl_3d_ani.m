% nH_skewColl_3d_ani.m
% Script to render simulation from nH_skewColl_3d.m as gif/mp4

% *** IMPORT WORKSPACE (from nH_skewColl_3d.m run) ***
...
% gif:
% filename = 'nH_skewColl_3d_ani.gif';

% mp4 video: 
filename = 'nH_skewColl_3d_ani.mp4'; v = VideoWriter(filename,'MPEG-4'); open(v);
...
%     scatter3(PHX((NP/2)+1:end,n),PHY((NP/2)+1:end,n),PHZ((NP/2)+1:end,n),psize,'filled','mo'); hold on; 
    ...
    % Title & Labels
    title('\textbf{neo-Hookean: Skew Impact of Balls}',['$t_{k}=$',' ',num2str(T(n),'%.2f')],'Interpreter','latex');
