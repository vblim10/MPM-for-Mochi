/*==========================================================
 * P2G_PIC_3d.c - grid to particle interpolation
 *
 * Interpolates grid values fp
 *       to particle values fg
 *
...
void P2G(const double Ox, const double Oy, const double Oz, // constants
         ...
         double *Xp, double *Yp, double *Zp, double *fp,    // input
            ...
            if(k>=0 && k<nz+1) {
                ...
                for (int S=0; S<49; S++) {
                    ...
                    if (i>=0 && i<nx+1 && j>=0 && j<ny+1) {
                        ...
                        // PIC transfer
                        fg[gidx] += fp[pidx]*( N_weight2(dx,bx) * N_weight2(dy,by) * N_weight2(dz,bz) )/ wh;                       
                    }
                }
            }
        }
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs!=16) {
        mexErrMsgIdAndTxt("P2G_PIC_3d:nrhs","16 inputs required.");
    }
    if (nlhs!=1) {
        mexErrMsgIdAndTxt("P2G_PIC_3d:nlhs","One output required.");
    }

    /* make sure the 10th input argument is type double */
    if (!mxIsDouble(prhs[9]) ||  mxIsComplex(prhs[9])) {
        mexErrMsgIdAndTxt("P2G_PIC_3d:notDouble","Xp must be type double.");
    }

    /* check that number of cols in input arguments is 1 */
    if (mxGetN(prhs[9])!=1 || mxGetN(prhs[10])!=1 || mxGetN(prhs[11])!=1) {
        mexErrMsgIdAndTxt("P2G_PIC_3d:notColVector","Xp, Yp, Zp must be column vectors.");
    }
   
    /* check that number of rows in input arguments is 1 */
    if (mxGetM(prhs[12])!=1) {
        mexErrMsgIdAndTxt("P2G_PIC_3d:notRowVector","fp must be a row vector.");
    }
    ...
    if (mxGetNumberOfElements(prhs[0])!=1  || mxGetNumberOfElements(prhs[1])!=1  ||
        ...
        mexErrMsgIdAndTxt("P2G_PIC_3d:notScalar","Inputs Ox, Oy, Oz, hx, hy, hz, nx, ny, nz, bx, by, bz must be scalars.");
    }
    ...
    /* create a pointer to the real data in Xp, Yp, Zp, fp */
